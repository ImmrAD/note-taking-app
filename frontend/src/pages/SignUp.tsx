...
const SignUp = () => {
  ...
  const handleGetOtp = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setOtpSent(true); // Update the state to show the OTP field
  };
  
  const handleSignUp = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    // ** In a real app, you would verify the OTP and create the account here **
    console.log("Signing up with OTP...");
    alert("Sign up successful!"); // Placeholder for success
  };
        ...
        <div className="w-full max-w-sm">
          ...
          <form className="w-full" onSubmit={handleSignUp}>
            ...
            <div className="mb-4">
              ...
              <input type="email" id="email" className={`w-full px-4 py-3 border rounded-lg text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100 ${otpSent ? 'border-gray-300' : 'border-2 border-blue-500'}`} defaultValue="jonas_kahnwald@gmail.com" disabled={otpSent}/>
            </div>
            ...
            {otpSent ? (
              <>
                <div className="mb-6">
                  ...
                  <div className="relative">
                    <input type="password" id="otp" className="w-full px-4 py-3 border-2 border-blue-500 rounded-lg text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 pr-10" placeholder="Enter OTP" required />
                    <div className="absolute inset-y-0 right-0 flex items-center pr-3 cursor-pointer"><EyeIcon /></div>
                  </div>
                </div>
                <button type="submit" className="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700 focus:outline-none focus:shadow-outline transition-colors duration-200">
                  Sign up
                </button>
              </>
            ) : (
              <button type="button" onClick={handleGetOtp} className="w-full mt-6 bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700 focus:outline-none focus:shadow-outline transition-colors duration-200">
                Get OTP
              </button>
            )}
          </form>
